    ...
    try {
      if (isSignUp) {
        ...
        if (data?.session) {
          ...
          alert(
            "Registration signal accepted. Please check your inbox to verify your account.",
          );
          navigate("/login");
        }
      } else {
      ...
      setError(
        err.message || "Uplink failed. Authentication server unreachable.",
      );
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#3b5a75]">
      <div className="win-panel w-full max-w-[400px] overflow-hidden shadow-2xl animate-aura-in">
        <div className="win-title-bar">
          ...
          <button
            onClick={() => navigate("/")}
            className="win-button py-0 px-2 h-4 bg-red-700 text-white font-bold"
          >
            X
          </button>
        </div>
        ...
        <div
          ...
          className="p-6 bg-[#d4d0c8] animate-aura-in transition-all duration-300"
        >
          <div className="flex gap-4 mb-6">
            <div className="w-12 h-12 bg-gray-400 border-2 border-white shadow-inner flex items-center justify-center flex-shrink-0 transition-transform hover:scale-105">
              <div
                className={`w-6 h-6 border-4 border-white rounded-full ${isSignUp ? "bg-blue-600" : "bg-transparent"}`}
              ></div>
            </div>
            <div>
              <h2 className="text-sm font-bold">
                {isSignUp ? "Identity Registration" : "Network Access"}
              </h2>
          ...
          {dbLinked === false && (
            <div className="mb-4 p-2 bg-yellow-100 border border-yellow-400 text-[9px] text-yellow-800 leading-tight animate-pulse">
              <b>UPLINK NOTICE:</b> Supabase connected but tables not detected.
              Please run the SQL in BACKEND_INTEGRATION.md to enable all
              features.
            </div>
          )}
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            <div className="flex items-center gap-4">
              <label className="w-20 text-[10px] font-bold text-right uppercase tracking-tighter">
                Auth ID:
              </label>
              <input
                type="email"
                ...
                className="flex-1 win-inset px-2 py-1 text-xs outline-none focus:bg-blue-50 border-gray-400 transition-colors"
              />
            </div>
            <div className="flex items-center gap-4">
              <label className="w-20 text-[10px] font-bold text-right uppercase tracking-tighter">
            ...
            <div className="pt-2">
              <button
                disabled={isLoading}
                className="win-button w-full font-bold text-xs h-9 bg-blue-100 hover:bg-blue-200 transition-all active:scale-[0.98]"
              >
                {isLoading
                  ? "ESTABLISHING..."
                  : isSignUp
          ...
          <div className="mt-6 border-t border-gray-400 pt-4 flex justify-between items-center">
            <Link
              ...
              className="text-[10px] underline text-blue-900 font-bold hover:text-blue-700 transition-colors"
            >
              {isSignUp ? "Existing user?" : "New user signup"}
            </Link>
            <button
              onClick={() => navigate("/")}
              className="win-button px-4 py-0.5 text-[10px]"
            >
              Cancel
            </button>
          </div>
