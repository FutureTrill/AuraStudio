...
import { generateWebsite, EngineType } from "../services/gemini.ts";
...
const StudioPage: React.FC = () => {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Aura_Core v1.0.4 Online. System is idle. Waiting for design parameters.",
      timestamp: new Date(),
    },
  ]);
    ...
    const initializeSession = async () => {
      if (!supabase) {
        const storedCount = parseInt(
          localStorage.getItem("aura_prompt_count") || "0",
          10,
        );
      ...
      try {
        ...
        if (session?.user) {
          ...
          if (latestProject) {
            ...
            if (latestProject.chat_history)
              setMessages(latestProject.chat_history);
          }
        } else {
          const storedCount = parseInt(
            localStorage.getItem("aura_prompt_count") || "0",
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const responseText = await generateWebsite(
        currentPrompt,
        chatHistory,
        selectedEngine,
      );
      ...
      const summary = summaryMatch
        ? summaryMatch[1].trim()
        : `Build complete using ${selectedEngine.toUpperCase()}.`;
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: `SYS_ERROR: ${error.message}`,
          timestamp: new Date(),
        },
      ]);
    } finally {
  ...
  return (
    <div
      className={`flex h-screen flex-col bg-[#d4d0c8] select-none overflow-hidden font-sans transition-all duration-1000 ${isShuttingDown ? "opacity-0" : "opacity-100"}`}
    >
      {isEntering && (
        <div className="fixed inset-0 bg-[#3b5a75] z-[10001] flex items-center justify-center animate-out fade-out duration-1000 fill-mode-forwards">
          <div className="text-white font-bold text-sm tracking-widest animate-pulse uppercase">
      ...
      <div className="flex bg-[#d4d0c8] p-1 border-b border-gray-400 gap-4 text-[11px] items-center">
        <button
          onClick={handleExit}
          className="flex items-center gap-1 font-bold px-3 py-0.5 bg-blue-800 text-white rounded-sm active:shadow-inner hover:bg-blue-700"
        >
        ...
        <div className="ml-auto flex items-center gap-3 pr-2 border-l border-gray-400 pl-3">
          <div className="flex items-center gap-2">
            <div
              className={`w-2 h-2 rounded-full ${supabase ? (userId ? "bg-green-500" : "bg-blue-500") : "bg-red-500"}`}
            ></div>
      ...
      <div className="flex-1 flex overflow-hidden p-1 gap-1 relative">
          ...
          <div className="absolute inset-y-0 left-0 w-[240px] bg-[#d4d0c8] border-r border-gray-400 z-50 shadow-2xl flex flex-col animate-in slide-in-from-left duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
              {projectHistory.length === 0 ? (
                <div className="text-[10px] italic text-gray-600 p-4">
                  No saved projects in this workspace.
                </div>
              ) : (
        ...
        <div className="w-[340px] flex flex-col gap-1">
          <div className="win-panel flex-1 flex flex-col overflow-hidden">
            <div className="win-title-bar">
              <div className="flex items-center gap-2">
                <span>Aura_Console</span>
              </div>
              <button
                onClick={() => setMessages([messages[0]])}
                className="w-4 h-4 win-button p-0 flex items-center justify-center text-[10px] bg-red-700 text-white font-bold"
              >
                X
              </button>
            </div>
            ...
            <div
              ref={scrollRef}
              className="flex-1 overflow-y-auto p-3 font-mono text-[11px] bg-black text-[#00ff00] custom-scrollbar shadow-inner selection:bg-green-900"
            >
              ...
              {isGenerating && (
                <div className="text-white animate-pulse">_PROCESSING...</div>
              )}
            </div>
            ...
            <div className="p-3 bg-[#d4d0c8] border-t border-gray-400 shadow-inner">
              <form onSubmit={handleSendMessage} className="space-y-2">
                <input
                  ...
                  className="w-full win-inset px-3 py-2 text-xs font-mono outline-none h-10 border-2 border-gray-500 focus:border-blue-600 transition-colors"
                />
                <button
                  type="submit"
                  disabled={isGenerating}
                  className="win-button w-full font-bold h-9 bg-gray-200 uppercase tracking-tight"
                >
                  Generate Build
                </button>
              </form>
          ...
          <div className="win-panel p-3 bg-[#d4d0c8]">
            <div className="text-[10px] font-bold border-b border-gray-500 pb-1 mb-3 uppercase tracking-tight text-gray-700">
              Resource Monitor
            </div>
            <div className="space-y-3">
              <div className="w-full h-4 bg-white border border-gray-500 p-[1px] relative">
                <div
                  className={`h-full bg-blue-800 transition-all duration-700 ${isGenerating ? "w-[85%]" : "w-[8%]"}`}
                ></div>
                <span className="absolute inset-0 flex items-center justify-center text-[8px] font-bold mix-blend-difference text-white uppercase">
                  Engine Load
                </span>
              </div>
              <div className="w-full h-4 bg-white border border-gray-500 p-[1px] relative">
                <div
                  className={`h-full ${userId ? "bg-green-600" : "bg-orange-600"} transition-all duration-500`}
                  style={{
                    width: userId ? "100%" : `${(promptCount / 3) * 100}%`,
                  }}
                ></div>
                <span className="absolute inset-0 flex items-center justify-center text-[8px] font-bold mix-blend-difference text-white uppercase">
                  Session Tokens
            ...
            <div className="win-title-bar">
              <div className="flex gap-1 items-end pt-1">
                <button
                  ...
                  className={`px-4 py-1 text-[10px] border-t border-x font-bold transition-all ${activeTab === "preview" ? "bg-[#d4d0c8] border-white text-black -mb-[1px] z-10" : "bg-gray-400 border-gray-600 text-gray-700 hover:bg-gray-300"}`}
                >
                  Visualizer
                </button>
                <button
                  onClick={() => setActiveTab("code")}
                  className={`px-4 py-1 text-[10px] border-t border-x font-bold transition-all ${activeTab === "code" ? "bg-[#d4d0c8] border-white text-black -mb-[1px] z-10" : "bg-gray-400 border-gray-600 text-gray-700 hover:bg-gray-300"}`}
                >
                  Source Viewer
                </button>
              </div>
              {activeTab === "code" && generatedCode && (
                <div className="flex gap-1 py-1 pr-1">
                  <button
                    ...
                    className="win-button py-0 px-2 h-4 text-[9px] bg-blue-100 font-bold uppercase"
                  >
                    Copy
                  </button>
                  <button
                    onClick={downloadCode}
                    className="win-button py-0 px-2 h-4 text-[9px] bg-green-100 font-bold uppercase"
                  >
                    Download
                  </button>
                </div>
              ...
              {activeTab === "preview" ? (
                generatedCode ? (
                  <iframe
                    ...
                    title="preview"
                    className="w-full h-full border-none animate-aura-in"
                  />
                ) : (
                  <div className="w-full h-full flex flex-col items-center justify-center text-center opacity-30 italic text-xs p-10 animate-pulse">
                    Waiting for design command to generate visualization.
                  </div>
                )
              ) : (
                <textarea
                  value={generatedCode}
                  readOnly
                  className="w-full h-full p-4 font-mono text-[11px] outline-none border-none resize-none bg-[#fafafa] custom-scrollbar selection:bg-blue-100"
                />
              )}
              {isGenerating && (
                <div className="absolute inset-0 bg-white/40 backdrop-blur-[1px] flex items-center justify-center z-20">
                  <div className="win-panel p-4 bg-[#d4d0c8] shadow-2xl flex flex-col items-center gap-2">
                    <div className="w-24 h-1 bg-gray-300 border border-gray-600 overflow-hidden">
                      <div className="h-full bg-blue-800 w-1/2 animate-loading-slide"></div>
                    </div>
      ...
      {showAuthGate && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999] backdrop-blur-[1px] animate-aura-in">
          <div className="win-panel w-[320px] shadow-2xl">
            <div className="win-title-bar bg-red-800">
              <div>Quota Reached</div>
            </div>
            <div className="p-5 bg-[#d4d0c8] space-y-4">
              <p className="text-[11px] font-bold">
                Standard session quota reached (3/3). Create an account to
                continue saving your workspace projects.
              </p>
              <div className="flex gap-2">
                <button
                  onClick={() => navigate("/login")}
                  className="win-button flex-1 font-bold py-2 bg-blue-100"
                >
                  Login / Signup
                </button>
                <button
                  onClick={() => setShowAuthGate(false)}
                  className="win-button flex-1 py-2"
                >
                  Close
                </button>
              </div>
